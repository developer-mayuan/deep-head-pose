...
#ifndef __imagetranslcm_image_t_hpp__
#define __imagetranslcm_image_t_hpp__

#include <vector>
#include <string>

namespace imagetranslcm
{
...
int image_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    if(this->image_size > 0) {
        tlen = __int8_t_encode_array(buf, offset + pos, maxlen - pos, &this->image[0], this->image_size);
        if(tlen < 0) return tlen; else pos += tlen;
    }

    char* info_cstr = (char*) this->info.c_str();
    tlen = __string_encode_array(buf, offset + pos, maxlen - pos, &info_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int image_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    if(this->image_size) {
        ...
        tlen = __int8_t_decode_array(buf, offset + pos, maxlen - pos, &this->image[0], this->image_size);
        if(tlen < 0) return tlen; else pos += tlen;
    }

    int32_t __info_len__;
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &__info_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__info_len__ > maxlen - pos) return -1;
    this->info.assign(((const char*)buf) + offset + pos, __info_len__ - 1);
    pos += __info_len__;
...
int image_t::_getEncodedSizeNoHash() const
{
    ...
    enc_size += __int16_t_encoded_array_size(NULL, 1);
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += __int8_t_encoded_array_size(NULL, this->image_size);
    enc_size += this->info.size() + 4 + 1;
    return enc_size;
}

uint64_t image_t::_computeHash(const __lcm_hash_ptr *)
{
    uint64_t hash = 0xab21409aa2ba96f8LL;
